...
for i in tqdm(range(len(onlyfiles))):
    ...
    igpd = iter(appended_daily_intraday.groupby('intrSn'))
    open = next(igpd)[1]
    mid = next(igpd)[1]
    close = next(igpd)[1]
    data_list = [open, mid, close]

    jumps = ['open/','mid/','close/']
    for j in range(len(jumps)):
        jump = jumps[j]
        appended_daily_intraday = data_list[j]
        try:appended_daily_intraday.to_pickle(out_path + jump + file)
        except:
            import os;os.mkdir(out_path + jump)
            appended_daily_intraday.to_pickle(out_path + jump + file)
