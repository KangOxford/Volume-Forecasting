...
for jump in ['open/','mid/','close/']:
    ...
    cols = pd.read_csv(path + jump[:-1]+ "_" +"ols_feat_15min.csv",index_col=0).index.to_list()[1:]
    ...
    for j in tqdm(range(len(onlyfiles))): # on mac4
        ...
        try:result.to_pickle(out_path + jump + file)
        except:import os;os.mkdir(out_path + jump);result.to_pickle(out_path + jump + file)
